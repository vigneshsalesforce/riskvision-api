const brewerySiteInspectionService = require('./../services/brewerySiteInspectionService');
...
const list = async (req, res) => {
    ...
    try {
        const data = await brewerySiteInspectionService.list(req, params); // Pass req and params
        res.json({ data: { data } });
    } catch (error) {
...
const create = async (req, res) => {
    try {
        const data = await brewerySiteInspectionService.create(req,req.body);
...
const update = async (req, res) => {
    const { id } = req.params;
    try {
        const data = await brewerySiteInspectionService.update(req, id, req.body);
       if(!data){
...
const view = async (req, res) => {
    const { id } = req.params;
    try {
        const data = await brewerySiteInspectionService.view(req, id);
      if(!data) {
         return res.status(404).json({ message: 'Property Risk Assessment not found' });
       }
        res.json({data:{data}});
...
const deleteRecord = async (req, res) => {
    const { id } = req.params;
    try {
        const data = await brewerySiteInspectionService.deleteRecord(req, id);
        if (!data) {
            return res.status(404).json({ message: 'Property Risk Assessment not found' });
        }
       res.json({data:{data}});
